...
const ListPet = () => {
  const { Pets, removePet, editPet, listPet } = useContext(GlobalContext);
    ...
    return (
      <Link to="/add">
        <button className="bg-green-400 hover:bg-green-500 text-white font-semibold py-2 px-4 rounded inline-flex items-center">
          ...
          <span className="pl-2">Add Pet</span>
        </button>
      </Link>
    );
  };
  ...
  return (
    <Fragment>
      <div className="flex flex-col justify-center items-center">
        ...
        {Pets.map((Pet) => (
          <div
            className="flex items-center bg-gray-100 shadow w-9/12 my-1"
            key={Pet.id}
          >
            <div className="flex-auto text-left px-4 py-2 m-2">
              <p className="text-gray-900 leading-none">{Pet.name}</p>
